<!DOCTYPE html>
<html lang="fr" dir="ltr">
    <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/materialize/1.0.0-rc.2/css/materialize.min.css">
        <link rel="stylesheet" href="assets/css/forum.css">
        <title>Forum</title>
    </head>
    <body>
        <nav>
            <div class="nav-wrapper">
                <ul id="nav-fixed" class="left hide-on-med-and-down">
                    <li><a href="loki.php" id="navMargin">LokiTheknigh</a></li>
                    <li><a href="index.php" id="navMargin">Devil-Boy</a></li>
                    <li><a href="video.php" id="navMargin">Video</a></li>
                    <li><a href="forum.php" id="navMargin">Forums</a></li>
                    <!-- Dropdown Trigger -->
                    <li><a class='dropdown-trigger btn' href='#' data-target='dropdown1'>Liste de Jeux</a></li>
                    <!-- Dropdown Structure -->
                </ul>
                    <ul id='dropdown1' class='dropdown-content'>
                        <li><a href="Overwatch.php">OverWatch</a></li>
                        <li><a href="Doom2.php">Doom 2</a></li>
                        <li><a href="MonsterHunter.php">Monster Hunter World</a></li>
                        <li><a href="infamous.php">Infamous Second Son</a></li>
                        <li><a href="fortnite.php">Fortnite</a></li>
                </ul>
            </div>

        </nav>
        <div class="parallax-container">
            <div class="parallax"><img src="assets/img/kitsune.jpg" /></div>
        </div>
        <p>


        </p>
        <div class="parallax-container">
            <div class="parallax"><img src="assets/img/girl_anime_night_comfort_book_103117_1920x1080.jpg" /></div>
        </div>
        <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" crossorigin="anonymous"></script>
        <script src="https://cdnjs.cloudflare.com/ajax/libs/materialize/1.0.0-rc.2/js/materialize.min.js"></script>
        <script src="assets/js/script.js"></script>
    </body>
</html>
